...
a = 0.058   #α
#a = -0.035
#beta_values = [0.01,0.001,-0.001,-0.01,-0.1,-1]
beta_values = [4*10**-2, 4*10**-3,4*10**-4,-4*10**-6,-4*10**-2,-4*10**-0]
#beta_values = [4*10**-2, 4*10**-3,0.0006,5*10**-4,-4*10**-6,-4*10**-2]
C = 8.36*10**8
sigma = 7700
...
for F in F_values:
    ...
    for beta in beta_values:
        ...
        print(f"\nRunning ensemble for F = {F} W/m^2 and β = {beta}".format(F))
        ...
        T_ensemble[(F, beta)] = T_members
        T_mean[(F, beta)] = np.mean(T_members, axis=0)

#-----Plotting--------------------------

plt.figure(figsize=(10,5))

colormaps = [plt.cm.Blues, plt.cm.Reds, plt.cm.Greens, plt.cm.Purples, plt.cm.Greys, plt.cm.Oranges]
combo_index = 0
...
for F in F_values:
    ...
    for beta in beta_values:
        ...
        shades = np.linspace(0.6, 0.8, ensemble_prints)
        ...
        plt.plot(time_years,T_mean[(F, beta)], color=cmap(0.95),linewidth=2,label=f"β = {beta}")

plt.xlabel("Time (years)")
plt.ylabel("ΔT (K)")
plt.title(f"Stochastic Temperature Response - F:{F}, α:{a}, λ: {lam}, σ:{sigma}, Ensemble per mean:{n_ensemble}")
plt.grid(True)
plt.xlim(0, years)
plt.legend()
