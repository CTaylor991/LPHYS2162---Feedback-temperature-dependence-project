...
for F in F_values:
    for beta in beta_values:
        ...
        for i in range(ensemble_prints):
            plt.plot(time_years, T_ensemble[(F, beta)][i],
                     linewidth=0.6, alpha=alpha)

        plt.plot(time_years, T_mean[(F, beta)],
                 linewidth=2.5,
                 label=f"Ensemble mean, F = {F}, β = {beta}")

plt.xlabel("Time (years)")
plt.ylabel("ΔT (K)")
plt.title(f"Stochastic Temperature Response - α:{a}, λ:{lam}, σ:{sigma}, Ensemble per F:{n_ensemble}")
plt.grid(True)
plt.xlim(0, years)
plt.legend()
