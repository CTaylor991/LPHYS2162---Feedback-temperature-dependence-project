...
-------------------------------------------------------------------------
DIFFERENT LAMBDA VALUE - STOCHASTIC RESPONSE
-------------------------------------------------------------------------
@author: charl
"""
...
lam_values  = [-1.28,-0.88, 0.5, 1]
#lam_values  = [-1,-0.5, 0.5, 1]
a = 0.058   #α
#a = -0.035
beta = -4*10**-6
C = 8.36*10**8
...
for F in F_values:
    ...
    for lam in lam_values:
        ...
        for i in range(ensemble_prints):
            plt.plot(time_years, T_ensemble[(F, lam)][i],color=cmap(shades[i]),
                     linewidth=0.6, alpha=alpha)

        plt.plot(time_years, T_mean[(F, lam)],color=cmap(0.95),
                 linewidth=2.5,
                 label=f"Ensemble mean, λ = {lam}")

plt.xlabel("Time (years)")
plt.ylabel("ΔT (K)")
plt.title(f"Stochastic Temperature Response -  F:{F}, α:{a}, β:{beta}, σ:{sigma}, Ensemble per mean:{n_ensemble}")
plt.grid(True)
plt.xlim(0, years)
plt.legend()
