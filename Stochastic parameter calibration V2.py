...
for run in range(n_ensemble):
    ...
    if milestone != int((run / n_ensemble) * 100):
        print(f"Run has completed: {milestone}%, Total time taken: {elapsed}s")
...
plt.xlabel("Time since spin-up (years)")
plt.ylabel("ΔT (K)")
plt.title(f"Example Ensemble Members (shifted to start at ΔT = 0) (σ={sigma}, {len(example_runs3)} of {n_ensemble} ensemble runs)")
plt.grid(True)
plt.xlim(0,years-spinup_years)
#plt.legend()
plt.tight_layout()
